...
<template>
	<div class="puzzle-container" v-show="isVerificationShow">
		<div class="puzzle-header">
			<!-- <span class="puzzle-header-left">{{$t('validatecode.tips')}}</span> -->
			<!-- <div>
        ...
        ref="puzzlelostbox"
			...
			<p :class="'ver-tips'+ (displayTips ? ' slider-tips' : '')" ref="verTips">
				<template v-if="verification">
					...
					<span style="color:#42ca6b;">{{$t('validatecode.success')}}</span>
					<span></span>
				</template>
				<template v-if="!verification">
					<i style="background-position:-4px -1229px;"></i>
					<span style="color:red;">{{$t('validatecode.error')}}</span>
				</template>
			</p>
		</div>

		<div class="slider-container" :style="'width:' + dataWidth + 'px;'">
			<div class="slider-bar">{{$t('validatecode.tips')}}</div>
