...
import loading from './su-loading'
...
export default {
  ...
  methods: {
    ...
    downloadMovByFirefox(url) {
      ...
      setTimeout(() => {
        if (_self.disabled) {
          _self.loading = loading.getLoading({
            text: '下载文件',
          })
        }
      }, 500)
    ...
    handleDownloadError(error) {
      if (error.response && error.response.status === 404) {
        this.$su.alert('下载失败，文件已失效！')
      } else if (error.message && error.message.includes('Network Error')) {
        this.$su.alert('下载失败，请检查网络连接！')
      } else {
        this.$su.alert('下载失败，请稍后再试！')
      }
    },
    getFileName(file) {
